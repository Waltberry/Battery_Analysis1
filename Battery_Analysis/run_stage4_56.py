# run_stage4_56.py (56-state version)
...
class Config:
    ...
    discharge_positive: bool = False  # discharge negative current (your dataset convention)
...
# =========================
# Dynamic index map (works for 56)
# =========================
...
# =========================
# Generic FVM builders (the same family as your 56-state test)
# =========================
def build_solid_fvm(N: int, D: float, R: float) -> np.ndarray:
    """
    Simple stable diffusion stencil; not identical to your legacy 4-state stencil.
    """
    s = D / (R ** 2)
...
def build_solid_input(N: int, cfg: Config, electrode: str) -> np.ndarray:
    """
    Input enters at surface state only (last CV).
    """
...
def build_electrolyte_1d(Nn: int, Ns: int, Np: int, cfg: Config) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # inject in first two CVs of negative region, last two of positive region
    b[0, 0] = s1
    if Nn > 1:
        b[1, 0] = s1
    b[-1, 0] = s3
...
def prepare_cycle_for_sysid(
    ...
    *,
    ...
    # Optional flip if using discharge positive convention
...
# =========================
# JAX models (Stage2 warm start + Stage4) 56-state
# =========================
def build_stage2_and_stage4_models(cfg: Config, idx: Dict[str, Any], theta_guard: float):
    ...
    USE_V0 = True  # keep your existing behavior
    ...
    def unpack_stage4(raw: jnp.ndarray):
        # theta: 5 entries (Gamma_p,Gamma_n,K_e,R0,V0), ab: 6 entries
        raw_theta = raw[:5]
        raw_ab = raw[5:11]
...
def main():
    ...
    # FIX: default matches control/mA
    ap.add_argument("--force_units", type=str, default="mA", help="A|mA|auto (auto -> guess)")
    ...
    CFG = Config()
    CFG.N_series = 1
    CFG.discharge_positive = False  # keep your convention
