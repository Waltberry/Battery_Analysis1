...
class Config:
    ...
    # -----------------------------
    # NEW: discretization sizes
    # -----------------------------
    # For 56 states: 16 + 16 + 24
    Nn: int = 16
    Np: int = 16
    Ne_n: int = 8
    Ne_s: int = 8
...
# =========================================================
# Generic FVM builders (same ones you tested in the notebook)
# NOTE: not identical to legacy 14-state stencil.
# =========================================================
def build_solid_fvm(N: int, D: float, R: float) -> np.ndarray:
    """
    Simple radial diffusion surrogate with tridiagonal-like structure.
    Not guaranteed to match your legacy 4x4 stencil; it is consistent + stable.
    """
    s = D / (R**2)
    A = np.zeros((N, N))
    # interior: second-difference-ish
    for i in range(1, N-1):
        A[i, i-1] = 1.0
        A[i, i]   = -2.0
        A[i, i+1] = 1.0

    # left boundary (symmetry-ish)
    A[0, 0] = -1.0
    A[0, 1] =  1.0
...
def build_solid_input(N: int, cfg: Config, electrode: str) -> np.ndarray:
    """
    Input enters at surface control volume only (last state of that electrode).
    """
    b = np.zeros((N, 1))
    if electrode == "n":
...
def build_electrolyte_1d(Nn: int, Ns: int, Np: int, cfg: Config) -> Tuple[np.ndarray, np.ndarray]:
    """
    1D through-cell electrolyte diffusion with 3 regions (n, sep, p),
    using simple nearest-neighbor coupling with region-aware spacing.
    """
    Ne = Nn + Ns + Np
    K = cfg.De / cfg.eps

    # define region lengths and cell widths
    ...
    # diffusion weights between neighbors i <-> i+1
    # using harmonic mean on dx for interface (simple stable choice)
    for i in range(Ne):
        if i > 0:
            dxL = dx(i-1)
            dxi = dx(i)
    ...
    # build B (source in negative electrolyte region and positive region)
    b = np.zeros((Ne, 1))
    ...
    # inject in first two CVs of neg region and last two CVs of pos region
    b[0, 0] = s1
    if Nn > 1:
        b[1, 0] = s1
    b[-1, 0] = s3
